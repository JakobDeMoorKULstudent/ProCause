...
def get_single_wssd_result(intervention, iter_gen, iter_eval, policy, delta, learner, model_type):
    ...
    # print it if it is a NaN
    if isinstance(wssd_result, float) and (wssd_result != wssd_result):  # Check for NaN
        print(f"NaN found for {intervention}, iter_gen: {iter_gen}, iter_eval: {iter_eval}, policy: {policy}, delta: {delta}, learner: {learner}, model_type: {model_type}")
