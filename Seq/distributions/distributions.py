...
from Seq.distributions.flows import sigmoid_flow, sigmoid_flow_inverse
...
class Categorical(BaseDistribution):
    ...
    def likelihood(self, x, params):
        """Computes log-likelihood for a categorical distribution."""
        log_probs = F.log_softmax(params, dim=-1)  # Convert logits to log-probabilities
        x = x.to(torch.int64)
        x = torch.argmax(x, dim=1)
        # return torch.gather(log_probs, dim=-1, index=x).squeeze(-1)
        return torch.gather(log_probs, dim=-1, index=x.unsqueeze(-1)).squeeze(-1)

    def sample(self, params, overlap=1):
        """Samples from a categorical distribution with u possible values."""
        probs = F.softmax(params, dim=-1)  # Convert logits to probabilities TODO: fix the overlap
    ...
    def mean(self, params):
        """Returns expected value of the categorical distribution (probabilities)."""
        return F.softmax(params, dim=-1)  # Convert logits to probabilities
...
class SigmoidFlow(BaseDistribution):
    ...
    def forward_transform(self, x, params, logdet=0):
        # print if x is an empty tensor
        if x.size(0) == 0:
            print("x is an empty tensor")
...
class MixedDistributionAtoms(BaseDistribution):
    ...
    def likelihood(self, x, params):
        ...
        # Check if only atoms or only continuous values are present
        close_to_atoms = torch.isclose(x[:, None], torch.tensor(self.atoms, dtype=x.dtype, device=x.device), atol=tolerance).any(dim=1)
        is_only_atoms = close_to_atoms.all()
        # is_only_atoms = torch.isclose(x[:, None], torch.tensor(self.atoms, dtype=x.dtype, device=x.device), atol=tolerance).any(dim=1).all()
        is_only_continuous = ~close_to_atoms.any()

        if not is_only_continuous:
