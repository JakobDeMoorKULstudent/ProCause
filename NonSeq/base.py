...
from NonSeq.utils import T, Y, to_np_vectors, to_np_vector, to_torch_variable, permutation_test
...
class BaseGenModel(object, metaclass=BaseGenModelMeta):
    ...
    def get_data(self, dataset=TRAIN, verbose=True):
        ...
        if dataset == TRAIN:
            x, t, y = self.data_train["X"], self.data_train["T"], self.data_train["Y"]
        elif dataset == VAL or dataset == "validation":
            x, t, y = self.data_val["X"], self.data_val["T"], self.data_val["Y"]
        elif dataset == TEST:
            x, t, y = self.data_test["X"], self.data_test["T"], self.data_test["Y"]
        else:
    ...
    def sample_x(self, dataset=TRAIN):
        if dataset == TEST:
            return self.data_test["X"]
        elif dataset == VAL:
            return self.data_val["X"]
        else:
            return self.data_train["X"]
    ...
    def sample_t(self, x, overlap=1):
        ...
        if x is None:
            x = self.sample_x()
        t = self._sample_t(x=x, overlap=overlap)
        return t

    def sample_y(self, x, t, causal_effect_scale=None,
        ...
        if x is None:
            x = self.sample_x()
        y_total = self._sample_y(t=t, x=x, ret_counterfactuals=True)
    ...
    def sample(self, x=None, dataset=TRAIN, overlap=1,
        ...
        """
        if x is None:
            x = self.sample_x(dataset=dataset)
        t = self.sample_t(x=x, overlap=overlap)
        if ret_counterfactuals:
            y_total = self.sample_y(x=x, t=t, causal_effect_scale=causal_effect_scale, deg_hetero=deg_hetero, ret_counterfactuals=True, seed=seed)
            return x, t, y_total
        else:
            y = self.sample_y(x=x, t=t, causal_effect_scale=causal_effect_scale, deg_hetero=deg_hetero, ret_counterfactuals=False, seed=seed)
            return x, t, y
    ...
    #     _, t_model, y_model = to_np_vectors(self.sample(seed=seed, dataset=dataset),
    #                                         thin_interval=thin_model)
    #     _, t_true, y_true = self.get_data(dataset=dataset, verbose=verbose)
    ...
    #                                  thin_model=None, thin_true=None, seed=None, n=None, outcome_distribution=None, t_only=False, y_only=False):
    #     """
    ...
    #     }
    #     """
    #     _, t_model, y_model = self.sample(dataset=dataset)
    ...
    def get_multivariate_quant_metrics(
        ...
        if t_model is None or y_model is None:
            _, t_model, y_model = self.sample(dataset=dataset)
        x_model, _, _ = self.sample(dataset=dataset)
        t_model = torch.argmax(t_model, dim=1) if self.dims["dim_t"] > 1 else t_model.squeeze(-1)
        old_y_shape = y_model.shape
        print('t_model.shape: ', t_model.shape)
        print('y_model.shape: ', y_model.shape)
        if n is not None and x_model.shape[0] > n:

            select_rows = np.random.choice(x_model.shape[0], n, replace=False)
            x_model = x_model[select_rows, :]
        ...
        x_true, t_true, y_true = self.get_data(dataset=dataset, verbose=verbose)
        t_true = torch.argmax(t_true, dim=1) if self.dims["dim_t"] > 1 else t_true.squeeze(-1)
        print('t_true.shape: ', t_true.shape)
        print('y_true.shape: ', y_true.shape)
        if n is not None and x_true.shape[0] > n:
            x_true = x_true[select_rows, :]
        ...
        if include_x:
            # flatten x_event
            model_samples = np.hstack((x_model, model_samples))
            true_samples = np.hstack((x_true, true_samples))
    ...
    def sample_bpic(self, nr_samples_per_case=50, seed=82*82):
        ...
        for sample in range(nr_samples_per_case):
            ...
            outcome_df = pd.DataFrame({"case_nr": data_prepped["case_nr"], "outcome0": [None] * len(data_prepped["case_nr"]), "outcome1": [None] * len(data_prepped["case_nr"]), "t_est": [None] * len(data_prepped["case_nr"])})
            X = data_prepped["X"]
            # get the prefix_len from X, by checking the index of the "prefix_len" column in prep_utils["column_names"] without case_nr and outcome, which is an 'Index' object
            # prefix_len_index = self.prep_utils["column_names"].get_loc("prefix_len")
            prefix_len_index = self.prep_utils["column_names"].drop(["outcome", "treatment", "case_nr"]).get_loc("prefix_len")
            prefix_len = X[:, prefix_len_index]
            # get unscaled prefix_len, and round to nearest integer
            prefix_len = np.round(self.prep_utils["scaler_dict_train"]["prefix_len"].inverse_transform(prefix_len.reshape(-1, 1)).flatten())
            # prefix_len = self.prep_utils["scaler_dict_train"]["prefix_len"].inverse_transform(prefix_len.reshape(-1, 1)).flatten()
            T = data_prepped["T"]
            _, t_est, y_total_ = self.sample(x=X, ret_counterfactuals=True, seed=new_seed)
    ...
    def calculate_metrics_bpic(self, estimated_outcome_df_list, ensemble=False):
        ...
        if ensemble:
            ...
            # estimated_outcome_df = pd.DataFrame()
            # for df in estimated_outcome_df_list:
            #     estimated_outcome_df = pd.concat([estimated_outcome_df, df], axis=0, ignore_index=True)
            # # take the mean for each case_nr/prefix_len
            # estimated_outcome_df = estimated_outcome_df.groupby(["case_nr", "prefix_len"]).mean().reset_index()
            # ensemble the estimated_outcome_df_list
        ...
        prefix_len_index = self.prep_utils["column_names"].drop(["outcome", "treatment", "case_nr"]).get_loc("prefix_len")
        prefix_len = data_prepped["X"][:, prefix_len_index]
        prefix_len = np.round(self.prep_utils["scaler_dict_train"]["prefix_len"].inverse_transform(prefix_len.reshape(-1, 1)).flatten())
        true_outcome_df = pd.DataFrame({"case_nr": data_prepped["case_nr"].numpy(), "y_true": data_prepped["Y"].flatten().numpy(), "t_true": data_prepped["T"].flatten().numpy(), "prefix_len": prefix_len})
