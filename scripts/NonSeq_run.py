...
parser = argparse.ArgumentParser(description='RealCause')
...
DATASET_PARAMS = {}
LOSS_TYPES = ["separate"]
PREP_PARAMS = {"test_prop": 0, "val_prop": 0.2, "standardize": True, "one_hot_encode": True, "impute": True, "seed": 0, "encoding": "aggregated"} 
...
if DATASET == "SimBank":
    ...
    MODEL_PARAMS["y"] = {"lr": 0.0001, "batch_size": 1024, "n_dense_y_x": 2, "dim_hidden": 25, "dim_y": 1, "outcome_distribution": ["normal", "atoms"]}
    ...
    if MODEL_PARAMS["causal_type"] == "T-Learner" or MODEL_PARAMS["causal_type"] == "S-Learner":
        MODEL_PARAMS["y"]["n_dense_y_x"] = 4
        MODEL_PARAMS["hidden_size_multiplier_dense_y"] = 1.15
    ...
    MODEL_PARAMS["t"] = {"lr": 0.0001, "batch_size": 1024, "n_dense_t": 4, "dim_hidden": 25, "dim_t": 1 if DATASET_PARAMS["action_width"][0] == 2 else DATASET_PARAMS["action_width"][0], 
                         "treatment_distribution": ["bernoulli"] if DATASET_PARAMS["action_width"][0] == 2 else ["categorical"]}
    MODEL_PARAMS["y"] = {"lr": 0.0001, "batch_size": 1024, "n_dense_y_x": 2, "dim_hidden": 25, "dim_y": 1, "outcome_distribution": ["bernoulli"]}
    MODEL_PARAMS["dim_sigmoidflow"] = 2
    MODEL_PARAMS["patience"] = 7500
    MODEL_PARAMS["grad_norm"] = 1.0
    MODEL_PARAMS["outcome_distribution"] = ["normal", "atoms"]
    ...
    PREP_PARAMS["filter_useless_cols"] = True
    PREP_PARAMS["last_state_cols"] = ["elapsed_time", "outcome"]
...
if not args.big_data:
    ...
    save_data(DATASET_PARAMS, os.path.join(os.getcwd(), DATA_FOLDER, DATASET, "dataset_params_" + str(DATASET_PARAMS["intervention_name"]) + BIAS_PATH + "RealCause" + PATH_SUFFIX_DATA))
    save_data(PREP_PARAMS, os.path.join(os.getcwd(), DATA_FOLDER, DATASET, "prep_params_" + str(DATASET_PARAMS["intervention_name"]) + BIAS_PATH + "RealCause" + PATH_SUFFIX_DATA))
...
for intervention in range(DATASET_PARAMS["nr_of_interventions"]):
    if not args.already_preprocessed:
        ...
        if args.big_data:
            save_data(data_train_prep, os.path.join(os.getcwd(), DATA_FOLDER, DATASET, "data_train_prep_" + str(DATASET_PARAMS["intervention_name"]) + BIAS_PATH + "RealCause" + PATH_SUFFIX_DATA))
            save_data(data_val_prep, os.path.join(os.getcwd(), DATA_FOLDER, DATASET, "data_val_prep_" + str(DATASET_PARAMS["intervention_name"]) + BIAS_PATH + "RealCause" + PATH_SUFFIX_DATA))
            save_data(data_test_prep, os.path.join(os.getcwd(), DATA_FOLDER, DATASET, "data_test_prep_" + str(DATASET_PARAMS["intervention_name"]) + BIAS_PATH + "RealCause" + PATH_SUFFIX_DATA))
            save_data(prep_utils, os.path.join(os.getcwd(), DATA_FOLDER, DATASET, "prep_utils_" + str(DATASET_PARAMS["intervention_name"]) + BIAS_PATH + "RealCause" + PATH_SUFFIX_DATA))
    else:
        data_train_prep = load_data(os.path.join(os.getcwd(), DATA_FOLDER, DATASET, "data_train_prep_" + str(DATASET_PARAMS["intervention_name"]) + BIAS_PATH + "RealCause" + PATH_SUFFIX_DATA))
        data_val_prep = load_data(os.path.join(os.getcwd(), DATA_FOLDER, DATASET, "data_val_prep_" + str(DATASET_PARAMS["intervention_name"]) + BIAS_PATH + "RealCause" + PATH_SUFFIX_DATA))
        data_test_prep = load_data(os.path.join(os.getcwd(), DATA_FOLDER, DATASET, "data_test_prep_" + str(DATASET_PARAMS["intervention_name"]) + BIAS_PATH + "RealCause" + PATH_SUFFIX_DATA))
        prep_utils = load_data(os.path.join(os.getcwd(), DATA_FOLDER, DATASET, "prep_utils_" + str(DATASET_PARAMS["intervention_name"]) + BIAS_PATH + "RealCause" + PATH_SUFFIX_DATA))
...
if args.tuning:
    for intervention in range(DATASET_PARAMS["nr_of_interventions"]):
        ...
        if not args.vsc:
            MODEL_PARAMS["savepath_y"] = "C:/Users/u0166838/Music/cache_best_model_y_TUNING_" + str(DATASET_PARAMS["intervention_name"]) + "RealCause" + BIAS_PATH + ".pt"
            MODEL_PARAMS["savepath_t"] = "C:/Users/u0166838/Music/cache_best_model_t_TUNING" + str(DATASET_PARAMS["intervention_name"]) + "RealCause" + BIAS_PATH + ".pt"
        else:
            MODEL_PARAMS["savepath_y"] = os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "cache_best_model_y_TUNING_" + str(DATASET_PARAMS["intervention_name"]) + "_VSC_RealCause" + BIAS_PATH + ".pt")
            MODEL_PARAMS["savepath_t"] = os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "cache_best_model_t_TUNING" + str(DATASET_PARAMS["intervention_name"]) + "_VSC_RealCause" + BIAS_PATH + ".pt")
        
        # split the validation data into two parts (one for training, and one for early stopping), do this via tensor 'case_nr' in the data
        data_train_tuning = {}
        data_val_tuning = {}
        data_test_tuning = {}
        ...
        tuner = HyperParamTuner(MODEL_PARAMS=MODEL_PARAMS,
                        ...
                        generator="RealCause", BIG_DATA=args.big_data, VSC=args.vsc, LOGGING=args.logging)
        
        for loss_type in LOSS_TYPES:
            MODEL_PARAMS["loss_type"] = loss_type
            print('\nTUNING T for Intervention', intervention, 'Loss Type', loss_type, "\n")
            
            if not args.t_already_tuned:
                best_params_t, best_network_t = tuner.tune_t()
                if args.big_data:
                    save_data(best_params_t, os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "best_params_t_tuning_" + str(DATASET_PARAMS["intervention_name"]) + "TarNet" + MODEL_PARAMS["loss_type"] + BIAS_PATH + "RealCause"))
                    torch.save(best_network_t.state_dict(), os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "best_model_t_tuning_" + str(DATASET_PARAMS["intervention_name"]) + "TarNet" + MODEL_PARAMS["loss_type"] + BIAS_PATH + "RealCause"))
            
            
            if not args.y_already_tuned:
                for learner in args.learners:
                    print('\nTUNING Y for Intervention', intervention, ', Loss Type', loss_type, ', Learner', learner, "\n")
                    ...
                    if tuner.MODEL_PARAMS["causal_type"] == "T-Learner" or tuner.MODEL_PARAMS["causal_type"] == "S-Learner":
                        tuner.MODEL_PARAMS["y"]["n_dense_y_x"] = 4
                        tuner.MODEL_PARAMS["hidden_size_multiplier_dense_y"] = 1.15

                    best_params_y, best_networks_y = tuner.tune_y()
                    if args.big_data:
                        save_data(best_params_y, os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "best_params_y_tuning_" + str(DATASET_PARAMS["intervention_name"]) + MODEL_PARAMS["causal_type"] + MODEL_PARAMS["loss_type"] + BIAS_PATH + "RealCause"))
                        torch.save([net.state_dict() for net in best_networks_y], os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "best_model_y_tuning_" + str(DATASET_PARAMS["intervention_name"]) + MODEL_PARAMS["causal_type"] + MODEL_PARAMS["loss_type"] + BIAS_PATH + "RealCause"))
else:
    for learner in args.learners:
        MODEL_PARAMS["causal_type"] = learner
        if MODEL_PARAMS["causal_type"] == "T-Learner" or MODEL_PARAMS["causal_type"] == "S-Learner":
        ...
        if not args.get_dfs_policies and not args.get_dfs_policies_only:
            best_params_t = load_data(os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "best_params_t_tuning_" + str(DATASET_PARAMS["intervention_name"]) + "TarNet" + "separate" + BIAS_PATH + "RealCause"))
            best_params_t = space_eval(TUNING_PARAMS_SPACE, best_params_t)
            best_params_y = load_data(os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "best_params_y_tuning_" + str(DATASET_PARAMS["intervention_name"]) + MODEL_PARAMS["causal_type"] + "separate" + BIAS_PATH + "RealCause"))
        ...
        for iteration in range(args.num_iterations):
            ...
            if not args.vsc:
                MODEL_PARAMS["savepath_y"] = "C:/Users/u0166838/Music/cache_final_model_y" + str(iteration) + str(DATASET_PARAMS["intervention_name"]) + "RealCause" + MODEL_PARAMS["causal_type"] + MODEL_PARAMS["loss_type"] + BIAS_PATH + ".pt"
                MODEL_PARAMS["savepath_t"] = "C:/Users/u0166838/Music/cache_final_model_t" + str(iteration) + str(DATASET_PARAMS["intervention_name"]) + "RealCause" + MODEL_PARAMS["causal_type"] + MODEL_PARAMS["loss_type"] + BIAS_PATH + ".pt"
            else:
                MODEL_PARAMS["savepath_y"] = os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "cache_final_model_y" + str(iteration) + str(DATASET_PARAMS["intervention_name"]) + MODEL_PARAMS["causal_type"] + MODEL_PARAMS["loss_type"] + "_VSC_RealCause" + BIAS_PATH + ".pt")
                MODEL_PARAMS["savepath_t"] = os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "cache_final_model_t" + str(iteration) + str(DATASET_PARAMS["intervention_name"]) + "_VSC_RealCause" + MODEL_PARAMS["causal_type"] + MODEL_PARAMS["loss_type"] + BIAS_PATH + ".pt")

            for intervention in range(DATASET_PARAMS["nr_of_interventions"]):
                print('\nTRAINING for Iteration', iteration, 'Intervention', intervention, "\n")
                ...
                realcause_generator = NonSeqGenerator(data_train=data_train_prep, 
                ...
                if args.already_trained:
                    if args.just_take_best:
                        for net, params in zip(realcause_generator.networks_y, torch.load(os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "best_model_y_tuning_" + str(DATASET_PARAMS["intervention_name"]) + MODEL_PARAMS["causal_type"] + MODEL_PARAMS["loss_type"] + BIAS_PATH + "RealCause"))):
                            net.load_state_dict(params)
                        realcause_generator.network_t.load_state_dict(torch.load(os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "best_model_t_tuning_" + str(DATASET_PARAMS["intervention_name"]) + "TarNet" + "separate" + BIAS_PATH + "RealCause")))
                    else:
                        if not args.get_dfs_policies and not args.get_dfs_policies_only:
                            for net, params in zip(realcause_generator.networks_y, torch.load(os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "final_model_y_" + str(DATASET_PARAMS["intervention_name"]) + MODEL_PARAMS["causal_type"] + MODEL_PARAMS["loss_type"] + BIAS_PATH + "RealCause" + str(iteration)))):
                                net.load_state_dict(params)
                            realcause_generator.network_t.load_state_dict(torch.load(os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "final_model_t_" + str(DATASET_PARAMS["intervention_name"]) + "TarNet" + "separate" + BIAS_PATH + "RealCause" + str(iteration))))
                            MODEL_PARAMS = load_data(os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "final_params_t_" + str(DATASET_PARAMS["intervention_name"]) + MODEL_PARAMS["causal_type"] + MODEL_PARAMS["loss_type"] + BIAS_PATH + "RealCause" + str(iteration)))
                else:
                    if not args.t_already_trained and learner == "TarNet":
                        # TRAIN MODEL T
                        print('Training T')
                        realcause_generator.train_t()

                        # SAVE MODEL T
                        if args.big_data:
                            torch.save(realcause_generator.network_t.state_dict(), os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "final_model_t_" + str(DATASET_PARAMS["intervention_name"]) + MODEL_PARAMS["causal_type"] + MODEL_PARAMS["loss_type"] + BIAS_PATH + "RealCause" + str(iteration)))
                            save_data(MODEL_PARAMS, os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "final_params_t_" + str(DATASET_PARAMS["intervention_name"]) + MODEL_PARAMS["causal_type"] + MODEL_PARAMS["loss_type"] + BIAS_PATH + "RealCause" + str(iteration)))

                    if not args.y_already_trained:
                        # TRAIN MODEL Y
                        print('Training Y', "\n")
                        realcause_generator.train_y()

                        # SAVE MODEL Y
                        if args.big_data:
                            torch.save([net.state_dict() for net in realcause_generator.networks_y], os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "final_model_y_" + str(DATASET_PARAMS["intervention_name"]) + MODEL_PARAMS["causal_type"] + MODEL_PARAMS["loss_type"] + BIAS_PATH + "RealCause" + str(iteration)))
                            save_data(MODEL_PARAMS, os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "final_params_t_" + str(DATASET_PARAMS["intervention_name"]) + MODEL_PARAMS["causal_type"] + MODEL_PARAMS["loss_type"] + BIAS_PATH + "RealCause" + str(iteration)))
                ...
                if args.testing:
                    ...
                    if DATASET == "SimBank":
                        ...
                        generator_classes = {"RealCause": realcause_generator}
                        GENERATOR_PREP_PARAMS = {"RealCause": PREP_PARAMS}
                        ...
                        simbank_evaluator = SimBankEvaluator(DATASET_PARAMS=DATASET_PARAMS, DATASET_PARAMS_INT=DATASET_PARAMS_LIST[intervention], 
                                                            ...
                                                            delta=delta, BIG_EVAL=args.big_eval, 
                                                            generator_iteration=iteration, get_dfs_policies=args.get_dfs_policies, get_dfs_policies_only=args.get_dfs_policies_only,
                        ...
                        if SAMPLE_FOR_STAT_TESTS:
                            _, cf_t_est_samples, cf_y_est_samples = realcause_generator.sample(dataset="test", ret_counterfactuals=True, seed=82)
                            _, t_est_samples, y_est_samples = realcause_generator.sample(dataset="test", seed=82)

                            if args.big_data:
                                save_data(cf_t_est_samples, os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "cf_t_est_samples_" + str(iteration) + "_" + str(DATASET_PARAMS["intervention_name"]) + MODEL_PARAMS["causal_type"] + BIAS_PATH + args.just_take_best_STRING + "RealCause"))
                                save_data(cf_y_est_samples, os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "cf_y_est_samples_" + str(iteration) + "_" + str(DATASET_PARAMS["intervention_name"]) + MODEL_PARAMS["causal_type"] + BIAS_PATH + args.just_take_best_STRING + "RealCause"))
                                save_data(t_est_samples, os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "t_est_samples_" + str(iteration) + "_" + str(DATASET_PARAMS["intervention_name"]) + MODEL_PARAMS["causal_type"] + BIAS_PATH + args.just_take_best_STRING + "RealCause"))
                                save_data(y_est_samples, os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "y_est_samples_" + str(iteration) + "_" + str(DATASET_PARAMS["intervention_name"]) + MODEL_PARAMS["causal_type"] + BIAS_PATH + args.just_take_best_STRING + "RealCause"))
                        ...
                        if EVALUATE_METRICS:
                            if SAMPLE_FOR_METRICS:
                                estimated_outcome_df = realcause_generator.sample_bpic(nr_samples_per_case=50, seed=82)
                                if args.big_data:
                                    save_data(estimated_outcome_df, os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "estimated_outcome_df_" + str(iteration) + "_" + str(DATASET_PARAMS["intervention_name"]) + MODEL_PARAMS["causal_type"] + BIAS_PATH + args.just_take_best_STRING + "RealCause"))
                            else:
                                estimated_outcome_df = load_data(os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "estimated_outcome_df_" + str(iteration) + "_" + str(DATASET_PARAMS["intervention_name"]) + MODEL_PARAMS["causal_type"] + BIAS_PATH + args.just_take_best_STRING + "RealCause"))
                            metrics = realcause_generator.calculate_metrics_bpic(estimated_outcome_df_list=estimated_outcome_df )
                            if args.big_data:
                                save_data(metrics, os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "metrics_" + str(iteration) + "_" + str(DATASET_PARAMS["intervention_name"]) + MODEL_PARAMS["causal_type"] + BIAS_PATH + args.just_take_best_STRING + "RealCause"))
if args.testing and 'bpic' in DATASET:
    # EVALUATE ENSEMBLE METRICS
    for iteration in range(args.num_iterations):
        estimated_outcome_df_list = []
        for learner in args.learners:
            estimated_outcome_df = load_data(os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "estimated_outcome_df_" + str(iteration) + "_" + str(DATASET_PARAMS["intervention_name"]) + learner + BIAS_PATH + args.just_take_best_STRING + "RealCause"))
            estimated_outcome_df_list.append(estimated_outcome_df)
        metrics_ensemble, merged_df = realcause_generator.calculate_metrics_bpic(estimated_outcome_df_list=estimated_outcome_df_list, ensemble=True)
        print('Metrics ensemble for iteration', iteration, metrics_ensemble, "\n")
        if args.big_data:
            save_data(metrics_ensemble, os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "metrics_ensemble_" + str(iteration) + "_" + str(DATASET_PARAMS["intervention_name"]) + BIAS_PATH + args.just_take_best_STRING + "RealCause"))
            save_data(merged_df, os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "merged_df_ensemble_" + str(iteration) + "_" + str(DATASET_PARAMS["intervention_name"]) + BIAS_PATH + args.just_take_best_STRING + "RealCause"))
    ...
    for learner in args.learners:
        ...
        t_est_samples_all = []
        for iteration in range(args.num_iterations):
            y_est_samples = load_data(os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "y_est_samples_" + str(iteration) + "_" + str(DATASET_PARAMS["intervention_name"]) + learner + BIAS_PATH + args.just_take_best_STRING + "RealCause"))
            y_est_samples_all.append(y_est_samples)
            t_est_samples = load_data(os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "t_est_samples_" + str(iteration) + "_" + str(DATASET_PARAMS["intervention_name"]) + "TarNet" + BIAS_PATH + args.just_take_best_STRING + "RealCause"))
        ...
        if args.big_data:
            save_data(t_est_samples_all, os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "t_est_samples_all_" + str(DATASET_PARAMS["intervention_name"]) + learner + BIAS_PATH + args.just_take_best_STRING + "RealCause"))
            save_data(y_est_samples_all, os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "y_est_samples_all_" + str(DATASET_PARAMS["intervention_name"]) + learner + BIAS_PATH + args.just_take_best_STRING + "RealCause"))

        uni_metrics_test, _, _ = realcause_generator.evaluate_statistical(y_model=y_est_samples_all, t_model=t_est_samples_all, only_univariate=True)
        if args.big_data:
            save_data(uni_metrics_test, os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "uni_metrics_test_all_" + str(DATASET_PARAMS["intervention_name"]) + learner + BIAS_PATH + args.just_take_best_STRING + "RealCause"))
        print('\n')
    ...
    if args.big_data:
        ...
        save_data(multi_metrics_test_no_x, os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "multi_metrics_test_ensemble_all_no_x_" + str(DATASET_PARAMS["intervention_name"]) + BIAS_PATH + args.just_take_best_STRING + "RealCause"))
        save_data(multi_metrics_test, os.path.join(os.getcwd(), RESULTS_FOLDER, DATASET, "multi_metrics_test_ensemble_all_" + str(DATASET_PARAMS["intervention_name"]) + BIAS_PATH + args.just_take_best_STRING + "RealCause"))
    print('\n')